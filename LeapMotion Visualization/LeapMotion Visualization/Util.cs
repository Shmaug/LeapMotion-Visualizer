...
namespace LeapMotion_Visualization
{
    static class Util
    {
        ...
        public static VertexPositionColor[] visualizeHands(Frame frame)
        {
            ...
            {
                Arm arm = hand.Arm;
                Vector3 elbow = Util.toWorld(arm.ElbowPosition);
                Vector3 armwrist = Util.toWorld(arm.WristPosition);
                Vector3 palm = Util.toWorld(hand.PalmPosition);
                Vector3 wrist = Util.toWorld(hand.WristPosition);
                ...
                {
                    Vector3 mcp = Util.toWorld(finger.JointPosition(Finger.FingerJoint.JOINT_MCP));
                    Vector3 pip = Util.toWorld(finger.JointPosition(Finger.FingerJoint.JOINT_PIP));
                    Vector3 dip = Util.toWorld(finger.JointPosition(Finger.FingerJoint.JOINT_DIP));
                    Vector3 tip = Util.toWorld(finger.JointPosition(Finger.FingerJoint.JOINT_TIP));
